...
`PROTECTED_JOB_NAMES` (`src/core/minions/protected-names.ts:15`) holds eight job types: seven run shell commands or spend model budget, while `ingest_capture` writes pages outside the ordinary operation layer. The single chokepoint is `MinionQueue.add` (`src/core/minions/queue.ts:81`), which throws unless the caller passes `{allowProtectedSubmit: true}`. The `submit_job` operation refuses to pass it for a protected name when the caller is remote (`src/core/operations.ts:2796`); the OAuth `/ingest` route opts in only after binding the queued destination to the verified token source and HMAC-signing that authorization with `GBRAIN_INGEST_QUEUE_HMAC_SECRET`. The worker verifies the signature and event content hash before writing, so unsigned pre-fix rows are quarantined as unrecoverable rather than replayed.

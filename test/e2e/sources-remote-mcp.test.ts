...
describeE2E('sources-remote-mcp E2E (gstack /setup-gbrain Path 4)', () => {
  ...
  test('OAuth /.well-known advertises all supported scopes', async () => {
    ...
    expect(meta.scopes_supported).toEqual(
      expect.arrayContaining(['admin', 'agent', 'context_mirror_recovery', 'read', 'sources_admin', 'users_admin', 'write']),
    );
  });

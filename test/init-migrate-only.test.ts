...
function run(args: string[]): { exitCode: number; stdout: string; stderr: string } {
  ...
  // GBRAIN_HOME is the product's cross-platform isolation boundary. HOME is
  // not authoritative on Windows, where os.homedir() may resolve USERPROFILE
  // and accidentally load the developer's real Brain config.
  const env = {
    ...process.env,
    HOME: tmp,
    GBRAIN_HOME: tmp,

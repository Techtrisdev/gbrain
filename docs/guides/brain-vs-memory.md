...
on new_information(info):
    ...
    if info.is_about_the_world:
        ...
        gbrain capture "..." --slug <slug>
        # Examples:
        #   "alice-example is CEO of acme-example"           -> gbrain (person page)
        #   "acme-example raised Series D at $12B"   -> gbrain (company page)
...
on user_asks(question):
    if question.about_person or question.about_company or question.about_meeting:
        ...
        # then fetch the cited page through the current agent/MCP get_page path
...
For downstream agents such as Hermes, also see the formal boundary decision in [`../architecture/hermes-gbrain-memory-boundary.md`](../architecture/hermes-gbrain-memory-boundary.md). <!-- fork-only link; strip before upstreaming -->

1. **Don't store people in agent memory.** "alice-example prefers email over Slack" feels like a preference, but it's a fact about alice-example -- it goes in GBrain on alice-example's page. Agent memory is for the agent's own operational state, not facts about people in the world.
...
1. Ask the agent "Who is alice-example?" -- confirm it runs `gbrain search` or the current `get_page`/MCP page-fetch path, not `memory_search`. Person lookup should hit GBrain.
...
5. After an agent reset, confirm GBrain knowledge is still accessible. Run `gbrain search <known-entity>` and then fetch the cited page through the current `get_page`/MCP path -- world knowledge should survive the reset.
